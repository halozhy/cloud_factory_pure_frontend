  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" align="center" min-width="200px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <!-- <el-button size="mini" type="primary" @click="handleBid(row)">
            竞标
          </el-button> -->
          <el-button size="mini" type="" @click="handleSeeBid(row)">
            查看
          </el-button>
          <el-button v-if="row.state === '已中标' || row.state === '已生产'" size="mini" type="primary" @click="handleSchedule(row)">
            排产
          </el-button>
          <el-button v-if="row.state === '未发货'" type="primary" size="mini" @click="handleDeliver(row)">
            发货
          </el-button>
    ...
    <el-dialog title="竞标" :visible.sync="dialogChooseBidFormVisible" class="pub_dialog" top="0">
      <el-table :data="listBids" border fit highlight-current-row style="width: 100%" max-height="400">
        ...
        <el-table-column label="竞标价格" min-width="100px" align="center">
          <template slot-scope="{row}">
            <span>{{ row.price }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" min-width="100px" class-name="small-padding fixed-width">
          <template slot-scope="{row}">
            <el-button :disabled="row.state === '已中标'" size="mini" type="primary" @click="handleBid(row)">
              中标
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title="查看竞标" :visible.sync="dialogSeeBidFormVisible">
      <el-form ref="dataForm" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="竞标价格">
          {{ bid_price }}
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" type="" @click="dialogSeeBidFormVisible = false">关闭</el-button>
      </span>
    </el-dialog>

    <el-dialog title="订单排产" :visible.sync="dialogScheduleVisible" width="1000px">
      <OrderSchedule :order-id="selectedOrderId" :user-id="user_id" />
    </el-dialog>
...
import OrderSchedule from './order-schdule'
...
export default {
  name: 'FacOrderMgr',
  components: { Pagination, OrderSchedule },
  directives: { waves },
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  created() {
    ...
    this.$axios.get('/api/fac/is_open', { params: { 'userId': this.user_id }}).then(r => {
      // alert(r)
      if (r.data !== 0) {
        this.$message.error('工厂被关停，不可接单生产')
        this.$router.push('/')
      } else {
        this.getList()
      }
  ...
  methods: {
    ...
    handleSeeBid(row) {
      ...
      // 先发一个请求看看有没有竞标过，这个接口是旧的，但是先不改了
      // 如果有竞标过，接口会返回竞标过的价格信息
      var info = {
        'user_id': this.user_id,
        'order_id': row.id
      }
      this.$axios.post('/api/bid/can_bid', info).then(r => {
        if (r.data.data === 1) {
          // this.$message.info('已竞标过，查看竞标信息')
          this.bid_price = r.data.price
        }
        this.dialogSeeBidFormVisible = true
      })
    },
    handleSchedule(row) {
      this.selectedOrderId = row.id
    ...
    handleDeliver(row) {
      this.$confirm('将发货, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$axios.get('/api/order/deliver', { params: {
          orderId: row.id
        }}).then(r => {
          // 0 ok
          if (r.data === 0) {
            ...
            this.$message.error('发货失败，不是未发货状态')
            this.handleReFresh()
          }
        })
      })
